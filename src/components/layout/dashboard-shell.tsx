...
function NavLinks({ role, onNavigate }: { role: DemoSession["role"]; onNavigate?: () => void }) {
  ...
  const renderGroup = (items: readonly { href: string; label: string; icon: typeof LayoutDashboard; permission: Permission }[]) => items.filter((item) => authorize(role, item.permission)).map((item) => {
    ...
    return (
      <Link
        ...
        className={cn(
          "flex h-10 items-center gap-3 px-3 text-xs uppercase tracking-[.12em] transition-colors",
          active ? "bg-white text-black" : "text-white/55 hover:bg-white hover:text-black",
        )}
      >
  ...
  return <nav className="flex flex-col"><p className="px-3 pb-2 pt-4 font-mono text-[10px] uppercase tracking-[.22em] text-white/35">Operação</p>{renderGroup(primaryNavigation)}<p className="px-3 pb-2 pt-6 font-mono text-[10px] uppercase tracking-[.22em] text-white/35">Crescimento</p>{renderGroup(growthNavigation)}</nav>;
}
...
export function DashboardShell({ children, session, notificationCount }: { children: React.ReactNode; session: DemoSession; notificationCount: number }) {
  ...
  return (
    <div className="relative min-h-screen bg-background">
      <div aria-hidden="true" className="surface-pattern" />
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-[244px] flex-col border-r border-white/15 bg-sidebar p-3 lg:flex">
      ...
      <div className="relative z-10 lg:pl-[244px]">
        <header className="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-white/15 bg-sidebar/95 px-4 backdrop-blur sm:px-6">
          <Sheet>
          ...
          <form action="/busca" className="relative hidden max-w-sm flex-1 sm:block">
            ...
            <Input aria-label="Buscar no sistema" name="q" placeholder="Buscar clientes, reservas..." className="rounded-none border-white/15 bg-transparent pl-9 dark:bg-transparent" />
          </form>
        ...
        <main className="surface-ambient mx-auto max-w-[1500px] px-5 pb-24 pt-10 lg:px-10 lg:pb-16">{children}</main>
      </div>
