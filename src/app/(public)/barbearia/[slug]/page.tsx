...
export default async function BusinessPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const reviewLabel = `${reviewCount} ${reviewCount === 1 ? "avaliação" : "avaliações"}`;
  ...
  return (
    <main className="min-h-screen overflow-x-clip bg-surface-base text-white">
      ...
      <header className="relative z-20 border-b border-white/10 bg-surface-base/95 backdrop-blur">
        <div className="mx-auto flex h-20 max-w-[1500px] items-center justify-between px-5 lg:px-10">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <Link
              href={bookingHref}
              className="group inline-flex h-10 items-center gap-2 whitespace-nowrap rounded-full bg-white px-5 text-[11px] font-semibold uppercase tracking-[.14em] text-black transition-colors hover:bg-surface-invert-muted"
            >
              <span className="sm:hidden">Agendar</span><span className="hidden sm:inline">Agendar horário</span> <ArrowUpRight className="size-3.5 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" />
            </Link>
          </div>
        </div>
      </header>
      ...
      >
        <p className="mb-8 flex items-center gap-2 text-[10px] uppercase tracking-[.2em] text-white/70"><span className="h-px w-8 bg-white/50" /> {business.city ?? "Barbearia"}</p>
        <h1 className="font-display text-[clamp(2.4rem,5.4vw,5.25rem)] uppercase leading-[.95] tracking-[-.055em]">Seu barbeiro,<br />na sua hora.</h1>
        <p className="mt-6 max-w-md text-base text-white/80">Marca online, chega na hora e senta. O resto é com a gente.</p>
        <Link href={bookingHref} className="mt-8 inline-flex min-h-12 w-fit items-center gap-5 rounded-lg bg-surface-invert px-6 text-sm font-medium text-black transition-colors hover:bg-surface-invert-muted">Agendar horário <ArrowRight className="size-4" /></Link>
        <p className="mt-4 font-mono text-[11px] uppercase tracking-[.16em] text-white/70">{nextSlot ? `Próximo horário livre: ${nextSlot}` : "Agenda aberta"}</p>
      ...
      <section className="mx-auto max-w-[1500px] px-5 lg:px-10">
        <div className="grid gap-5 border-b border-white/12 px-1 py-6 sm:grid-cols-3 sm:gap-8">
          <div className="flex items-center gap-3"><Star className="size-4" /><div><p className="text-sm">{rating ? `${rating.toFixed(1)} / 5 · ${reviewLabel}` : "Ainda sem avaliações"}</p><p className="mt-1 text-xs text-white/65">{rating ? "De quem já sentou aqui" : "As primeiras aparecem aqui"}</p></div></div>
          <div className="flex items-center gap-3"><Clock3 className="size-4" /><div><p className="text-sm">Marca em um minuto</p><p className="mt-1 text-xs text-white/65">Sem criar conta, sem pagar sinal</p></div></div>
      ...
      <section id="servicos" className="scroll-mt-20 bg-surface-invert text-[#20201e]">
        <div className="mx-auto max-w-[1500px] px-5 py-14 lg:px-10 lg:py-20">
          <div className="mb-9 flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
            <div><p className="mb-3 text-xs uppercase tracking-[.18em] text-black/65">Serviços</p><h2 className="font-heading text-4xl font-semibold tracking-[-.055em] sm:text-5xl">Serviços e preços. Sem surpresa.</h2></div>
            <p className="max-w-xs text-sm leading-6 text-black/65">Na dúvida, pergunta pro barbeiro.<br />Ele ajuda a escolher.</p>
          </div>
          <div className="grid gap-8 lg:grid-cols-[1.25fr_.75fr] lg:gap-14">
            <div className="border-t border-black/15">
              {business.services.map((service) => (
                <Link key={service.id} href={`${bookingHref}?servico=${service.id}`} className="group grid grid-cols-[1fr_auto] items-center gap-4 border-b border-black/15 px-2 py-6 transition-colors hover:bg-black/5 sm:grid-cols-[1fr_auto_auto]">
                  <div><h3 className="font-heading text-lg font-semibold tracking-tight sm:text-xl">{service.name}</h3>{service.description ? <p className="mt-1 text-xs leading-5 text-black/65">{service.description}</p> : null}<p className="mt-2 text-xs text-black/65 sm:hidden">{service.durationMinutes} min</p></div>
            ...
            <figure className="relative min-h-72 overflow-hidden lg:min-h-full"><Image src="/images/barber-detail.png" alt="Detalhe do acabamento de uma barba com navalha" fill sizes="(max-width: 1024px) 100vw, 40vw" className="object-cover" /><figcaption className="absolute bottom-5 left-5 rounded-md bg-black/65 px-3 py-2 text-xs text-white">Navalha e acabamento à mão.</figcaption></figure>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-[1500px] px-5 py-20 lg:px-10 lg:py-28">
          <div className="mb-10 flex items-end justify-between border-b border-white/15 pb-8">
            <div>
              ...
              <h2 className="font-display text-5xl uppercase leading-[.85] tracking-[-.055em] sm:text-7xl lg:text-8xl">Veja antes<br />de sentar.</h2>
            </div>
          ...
          <div className="grid auto-rows-[220px] gap-3 sm:grid-cols-2 sm:auto-rows-[300px] lg:grid-cols-12 lg:auto-rows-[250px]">
            ...
            <div className="flex flex-col justify-between bg-white p-7 text-black sm:row-span-1 lg:col-span-5">
              ...
              <div>
                <p className="mb-3 text-[10px] uppercase tracking-[.22em] text-black/65">Regra da casa</p>
                <p className="font-heading max-w-sm text-2xl font-semibold leading-tight tracking-[-.04em]">Primeiro a conversa.<br />Depois a tesoura.</p>
              </div>
            </div>
        ...
        <div className="mx-auto grid max-w-[1500px] gap-12 px-5 py-20 lg:grid-cols-[1fr_1fr] lg:px-10 lg:py-28">
          ...
          <div className="flex flex-col justify-between lg:py-4">
            <div>
              ...
              <h2 className="font-display text-5xl uppercase leading-[.85] tracking-[-.055em] sm:text-7xl">Bom corte.<br />Boa conversa.{business.city ? <><br />Em {business.city}.</> : null}</h2>
              <p className="mt-8 max-w-xl text-base leading-7 text-black/60">
                {business.description ?? `${business.name} atende com hora marcada${business.city ? ` em ${business.city}` : ""}. Escolha o barbeiro e o horário aqui pelo site.`}
              </p>
            </div>
            <div className="mt-12 grid grid-cols-2 border-y border-black/20">
              {rating ? (
                <>
                  <div className="border-r border-black/20 py-6 pr-6"><p className="font-display text-5xl tracking-[-.05em]">{rating.toFixed(1)}</p><p className="mt-2 text-[10px] uppercase tracking-[.16em] text-black/65">Nota média</p></div>
                  <div className="py-6 pl-6"><p className="font-display text-5xl tracking-[-.05em]">{reviewCount}</p><p className="mt-2 text-[10px] uppercase tracking-[.16em] text-black/65">{reviewCount === 1 ? "Avaliação" : "Avaliações"}</p></div>
                </>
              ) : (
                <>
                  <div className="border-r border-black/20 py-6 pr-6"><p className="font-display text-5xl tracking-[-.05em]">{business.staff.length}</p><p className="mt-2 text-[10px] uppercase tracking-[.16em] text-black/65">{business.staff.length === 1 ? "Barbeiro" : "Barbeiros"}</p></div>
                  <div className="py-6 pl-6"><p className="font-display text-5xl tracking-[-.05em]">{business.services.length}</p><p className="mt-2 text-[10px] uppercase tracking-[.16em] text-black/65">{business.services.length === 1 ? "Serviço" : "Serviços"}</p></div>
                </>
              )}
            </div>
          </div>
      ...
      <section id="equipe" className="surface-ambient scroll-mt-20 border-t border-white/10 bg-surface-panel">
        <div className="mx-auto max-w-[1500px] px-5 py-20 lg:px-10 lg:py-28">
          <div className="mb-10 grid gap-6 lg:grid-cols-2">
            ...
            <p className="max-w-md self-end text-sm leading-6 text-white/65 lg:justify-self-end">Cada um tem o seu jeito. Escolhe o seu.</p>
          </div>
              ...
              <Link key={member.id} href={`${bookingHref}?profissional=${member.id}`} className="group flex flex-col overflow-hidden rounded-xl border border-white/12 bg-surface-raised transition-colors hover:bg-surface-invert hover:text-black">
                ...
                <div className="flex items-start justify-between gap-3 p-6">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-current/70">{member.title ?? "Barbeiro"}</p>
                    {member.rating ? (
        ...
        <div className="mx-auto max-w-[1500px] px-5 py-20 lg:px-10 lg:py-28">
          <div className="grid gap-10 lg:grid-cols-[.55fr_1.45fr]">
            <div>
              ...
              <div className="mt-8 flex items-center gap-3"><div className="flex gap-0.5">{Array.from({ length: 5 }).map((_, index) => <Star key={index} className="size-3.5 fill-black" />)}</div><span className="font-mono text-xs">{rating ? `${rating.toFixed(1)} / 5 em ${reviewLabel}` : "Ainda sem avaliações"}</span></div>
            </div>
          ...
          <div className="mt-20 grid overflow-hidden bg-black text-white lg:grid-cols-[1fr_auto]">
            <div className="p-8 sm:p-12"><p className="mb-5 text-[10px] uppercase tracking-[.2em] text-white/65">{nextSlot ? "Próximo horário livre" : "Agenda aberta"}</p><h3 className="font-display max-w-4xl text-5xl uppercase leading-[.85] tracking-[-.055em] sm:text-7xl">{nextSlot ? <>{nextSlot}.</> : <>Escolha<br />seu horário.</>}</h3></div>
            <Link href={bookingHref} className="group flex min-h-48 items-center justify-center gap-3 border-t border-white/20 px-10 text-xs font-semibold uppercase tracking-[.16em] transition-colors hover:bg-white hover:text-black lg:min-h-full lg:border-l lg:border-t-0">
              Agendar horário <ArrowUpRight className="size-4 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" />
            </Link>
          </div>
        </div>
      </section>
      ...
      <section id="produtos" className="surface-ambient scroll-mt-20 border-t border-white/10 bg-surface-base">
        <div className="mx-auto max-w-[1500px] px-5 py-20 lg:px-10 lg:py-28">
          <div className="mb-10 grid gap-6 lg:grid-cols-2">
            <div><p className="mb-4 text-[10px] uppercase tracking-[.22em] text-white/65">06 / Produtos</p><h2 className="font-display text-5xl uppercase leading-[.85] tracking-[-.055em] sm:text-7xl">O que usamos<br />em você.</h2></div>
            <p className="max-w-md self-end text-sm leading-6 text-white/65 lg:justify-self-end">Está no balcão. É só pedir.</p>
          </div>
        ...
        <div className="mx-auto max-w-[1500px] px-5 py-20 lg:px-10 lg:py-28">
          <div className="grid gap-12 border-t border-white/15 pt-10 lg:grid-cols-2">
            <div>
              ...
              <Link href={bookingHref} className="group mt-10 inline-flex h-14 items-center gap-4 rounded-full bg-white px-7 text-xs font-semibold uppercase tracking-[.16em] text-black hover:bg-surface-invert-muted">Agendar horário <ArrowRight className="size-4 transition-transform group-hover:translate-x-1" /></Link>
            </div>
